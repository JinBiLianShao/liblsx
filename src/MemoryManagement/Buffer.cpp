...
#include <mutex>    // Already included in header, but good practice if using unique_lock etc.
...
size_t Buffer::WriteAt(size_t offset, const uint8_t* data, size_t size) {
     ...
     std::lock_guard<std::mutex> lock(mutex_); // Protect data_vector_ and its data pointer
