...
uint8_t* FixedSizeQueue::get_block_address(size_t index) {
    ...
    // index should already be within [0, block_count_ - 1] if head_ and tail_ are managed correctly
    if (index >= block_count_) {
         // This should ideally not happen if logic is correct
         std::cerr << "FixedSizeQueue: get_block_address called with out-of-bounds index: " << index << std::endl;
         throw std::out_of_range("FixedSizeQueue: Block index out of range in get_block_address. This indicates a logical error in head/tail management.");
    }
...
const uint8_t* FixedSizeQueue::get_block_address(size_t index) const {
    // No lock here, assumes caller holds the lock
    if (index >= block_count_) {
         std::cerr << "FixedSizeQueue: get_block_address (const) called with out-of-bounds index: " << index << std::endl;
         throw std::out_of_range("FixedSizeQueue: Block index out of range in get_block_address (const). This indicates a logical error in head/tail management.");
    }
    return buffer_.data() + index * block_size_;
}
...
FixedSizeQueue::FixedSizeQueue(size_t block_size, size_t block_count)
    : block_size_(block_size), block_count_(block_count), head_(0), tail_(0), current_size_(0) // Initialize all members
{
    ...
    try {
        ...
        block_size_ = 0;
        block_count_ = 0;
        // head_, tail_, current_size_ already 0 from member initializer list
        throw;
    }
}
