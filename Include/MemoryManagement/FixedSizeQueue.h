 ...
 * - **FIFO 顺序**: 保证块的存取顺序是先进先出。
 * - **循环缓冲区**: 利用底层线性缓冲区模拟循环队列的行为，高效利用内存。
 ...
 * for (int i = 0; i < 12; ++i) {
 * // 填充数据，例如块编号
 * std::iota(data.begin(), data.end(), static_cast<uint8_t>(i + id * 50));
 * std::cout << "[Producer " << id << "] Attempting to put block " << i << "..." << std::endl;
 * // 使用阻塞 Put，等待队列有空间，最多等待 100ms
 * if (queue.PutBlocking(data, 100)) {
 ...
 * }
 * std::this_thread::sleep_for(std::chrono::milliseconds(40)); // 短暂休息
 * }
 * }
 *
 * // 消费者线程
 ...
 * // 使用阻塞 Get，等待队列有数据，最多等待 200ms
 * if (queue.GetBlocking(buffer, buffer.size(), 200)) {
 ...
 * }
 * std::this_thread::sleep_for(std::chrono::milliseconds(120)); // 模拟处理时间
 * }
 * }
 *
 * int main() {
 ...
 * // 示例：非阻塞操作
 * queue.Clear();
 * std::vector<uint8_t> test_data(queue.BlockSize(), 0xCC);
...
namespace LIB_LSX {
    ...
    namespace Memory {
        ...
        /**
         * @brief 固定大小内存块队列类。
         * 实现一个线程安全的、存储固定大小内存块的 FIFO 队列。
         * 支持非阻塞和阻塞（带超时）的放入和取出操作。
         */
        // 7. FixedSizeQueue Module (固定大小块队列模块)
        // 实现存储固定大小内存块的 FIFO 队列
        ...
        private:
            ...
            /**
             * @brief 队列头部索引。
             * 指向队列中最旧的有效块的起始位置（以块为单位的索引，0 到 block_count_ - 1）。
             */
            size_t head_ = 0; // Index of the first valid block (0 to block_count_ - 1)
            /**
             * @brief 队列尾部索引。
        ...
        public:
            ...
            FixedSizeQueue(size_t block_size, size_t block_count);
            /**
             * @brief 析构函数。
             ...
             * 使队列回到初始的空状态。
             */
            // Clear the queue (resets head/tail/size)
            void Clear();
             ...
             */
            // Put a fixed-size block into the queue (Enqueue)
             ...
             */
            // Get a fixed-size block from the queue (Dequeue)
             ...
             * @return 队列中当前存储的块数量。
             */
            // Get the number of blocks currently in the queue
            size_t Size() const;
             ...
             * @return 队列的最大块数量。
             */
            // Get the maximum number of blocks the queue can hold
